...
* **最後の期間に変更されていない共有ファイルの検疫** - 最近変更されていない共有ファイルに関するアラートを受信し、検疫を行うか、自動アクションを有効にすることを選択します。 指定した日付範囲内に変更されなかったプライベート ファイルをすべて除外します。 Google Workspace では、ポリシー作成ページの [検疫ファイル] チェックボックスを使用して、これらのファイルを検疫するように選択できます。
