...
> [!IMPORTANT]
> Cloud App Security に Microsoft Defender for Identity を統合し、アラート通知を SIEM に送信するように両方のサービスを構成している場合、同じアラートに対して重複する SIEM 通知が送られてきます。 各サービスからはアラートが 1 つ発行され、異なるアラート ID が与えられます。 重複や混乱を避けるため、このシナリオに対処してください。 たとえば、アラート管理を実行する箇所を決定し、他のサービスからの SIEM 通知の送信を停止します。
