...
1. [SIEM エージェント] タブで、[[新しい SIEM エージェントの追加]](siem.md) をクリックし、ウィザードを使用して、SIEM への接続の詳細を構成します。 たとえば、次の構成で新しい SIEM エージェントを作成できます。
    - **Protocol**:TCP
    - **[リモート ホスト]** : ポートをリッスンできる任意のコンピューター。 たとえば、シンプルなソリューションでは、エージェントと同じコンピューターを使用し、リモート ホストの IP アドレスを 127.0.0.1 に設定します
    - **[ポート]** :リモート ホスト コンピューターでリッスンできる任意のポート
...
1. SIEM がデータを受信しない場合は、新しい SIEM エージェント コンピューターで、アクティビティを転送するように構成したポートをリッスンして、データがエージェントから SIEM に送信されているかどうかを確認します。 たとえば、`netcat -l <port>` を実行します。ここで `<port>` は以前に構成したポート番号です。
