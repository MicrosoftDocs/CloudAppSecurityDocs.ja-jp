...
ms.locfileid: "88779476"
---
# <a name="network-requirements"></a>ネットワークの要件

*適用対象:Microsoft Cloud App Security*

この記事には、Microsoft Cloud App Security を使用するために許可する必要があるポートおよび IP アドレスの一覧と、許可リストが記載されています。
...
> [!NOTE]
> ワイルドカード (\*) の代わりに、特定のテナント URL のみを開くことができます。たとえば、上のスクリーンショットに基づいて、次を開くことができます: mod244533.us.portal.cloudappsecurity.com

## <a name="access-and-session-controls"></a>アクセス制御とセッション制御

お使いの環境に関連する設定を使用して、リバース プロキシ用にファイアウォールを構成します。
...
Cloud App Security からアクセスされる一部のサード パーティ製アプリでは、次の IP アドレスが使用される場合があります。 この IP アドレスにより、Cloud App Security によるログ収集や、Cloud App Security コンソールへのアクセスが可能になります。

> [!NOTE]
> これらの IP アドレスは、ベンダーからのアクティビティ ログに表示されることがあります。Cloud App Security によって、これらの IP アドレスからガバナンス アクションとスキャンが実行されるためです。

サード パーティ製のアプリに接続するには、次の IP アドレスからの Cloud App Security による接続を有効にします。
...
## <a name="third-party-dlp-integration"></a>サード パーティ DLP 統合
...
サード パーティ製のアプリに接続し、外部 DLP ソリューションと統合するには、次の IP アドレスからの Cloud App Security による接続を有効にします。
...
既定のテンプレートと設定から通知を送信できるようにするには、次の IP アドレスをスパム対策の許可リストに追加します。 Cloud App Security 専用のメール IP アドレスは次のとおりです。
...
メール送信者 ID をカスタマイズする場合、Microsoft Cloud App Security では、サード パーティ製のメール サービスである MailChimp&reg; を使用したカスタマイズが可能です。 これを機能させるには、Microsoft Cloud App Security ポータルで **[設定]** に移動します。 **[メールの設定]** を選択し、MailChimp のサービス使用条件とプライバシーに関する声明を確認します。 次に、お客様に代わって MailChimp を使用するためのアクセス許可を Microsoft に付与します。

送信者の ID をカスタマイズしない場合は、すべての既定の設定を使用してメール通知が送信されます。

MailChimp を使用するには、次の IP アドレスをスパム対策の許可リストに追加して、通知を送信できるようにします。198.2.134.139 (mail1.cloudappsecurity.com)

## <a name="log-collector"></a>ログ コレクター

ログ コレクターを使用した Cloud Discovery 機能を有効にし、組織内のシャドウ IT を検出するには、次の項目を開きます。

- ログ コレクターが着信 FTP および Syslog トラフィックを受信できる
- ログ コレクターがポート 443 でポータル (contoso.cloudappsecurity.com など) への発信トラフィックを開始できる
- ログ コレクターが、ポート 443 で Azure BLOB ストレージへの送信トラフィックを開始できるようにする。
